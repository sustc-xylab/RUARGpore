...
echo "ARGpore is runing using parameters:
...
Query="${Input_fa2}"
bash $DIR/bin/centrifuge_marker_chenghang.sh $Query $N_threads $DIR $Simcutoff $Lencutoff $nowt $barcode
...
"
out=`echo "${Input_fa2}_ARGpore_${nowt}"`
if [ ! -d $out ]; then 
	mkdir $out;
